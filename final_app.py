...
def get_local_currency(city_string, index_val, category, col_index=100):
    try:
    ...
    if category == "Rent":
        ...
        # FIXED MATH: True Salary factors in both Purchasing Power AND local Cost of Living
        usd_val = (index_val / 100) * (col_index / 100) * NYC_BASE_INCOME
    else: 
...
# NEW: Info box explaining the NYC Baseline to laymen
st.info("ℹ️ **Data Note:** All Index values on this dashboard are calculated using **New York City as the global baseline (where NYC = 100)**. For example, a Rent Index of 50 means the rent is estimated to be 50% of the cost in New York City.")
...
c3.caption(f"**Est. Net Salary:** {get_local_currency(selected_city, city_data['Local Purchasing Power Index'], 'Income', city_data['Cost of Living Index'])}")

c4.metric("Total Cost of Living Index", f"{city_data['Cost of Living Index']:.1f}")
c4.caption(f"**Est. Total/Month:** {get_local_currency(selected_city, city_data['Cost of Living Index'], 'Cost of Living')}")
...
m3.caption(f"**Est. Net Salary:** {get_local_currency(selected_city, pred_power, 'Income', final_prediction)}")
...
m4.caption(f"**Est. Total/Month:** {get_local_currency(selected_city, final_prediction, 'Cost of Living')}")
...
if submitted and user_query:
    context_str = (
        ...
        f"Predicted Cost of Living: {final_prediction:.1f}, Rent: {pred_rent:.1f}. "
    )
...
col_verdict, col_desc = interpret_index(final_prediction, "Cost of Living")
...
with st.container(border=True):
    ...
    # Layman's Sentence Generation
    est_income = get_local_currency(selected_city, pred_power, 'Income', final_prediction)
    est_rent = get_local_currency(selected_city, pred_rent, 'Rent')
    est_col = get_local_currency(selected_city, final_prediction, 'Cost of Living')
