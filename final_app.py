...
def load_system_assets():
    # 1. Define the filenames exactly as they appear in GitHub
    model_name = "best_cost_of_living_model.pkl"
    data_name = "city_stats_with_coords.csv"
    
    # 2. Get the directory of the current script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 3. Create a list of potential full paths for the model
    # We check the root, the script dir, and the current working dir
    potential_model_paths = [
        os.path.join(script_dir, model_name),
        os.path.join(os.getcwd(), model_name),
        model_name  # Just the filename
    ]
    ...
    for path in potential_model_paths:
        if os.path.exists(path):
            try:
                ...
                # Once model is found, check for the CSV in that same folder
                csv_path = path.replace(model_name, data_name)
                if os.path.exists(csv_path):
                    stats = pd.read_csv(csv_path)
                    print(f"✅ Loaded assets from: {path}")
                    return model, stats
            except Exception as e:
